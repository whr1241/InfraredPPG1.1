...
def objectTracking(video_path, play_realtime=False):
    ...
    for i in range(1,n_frame):
        ...
        # mean_gray.append(np.mean(cv2.cvtColor(frames[i],cv2.COLOR_RGB2GRAY)[ymin:ymin+boxh,xmin:xmin+boxw]))
        # show_frame, levels_mean, background = multiscale_value(xmin, ymin, boxw, boxh, frames[i])
        show_frame, levels_mean = gridding_value(xmin, ymin, boxw, boxh, frames[i])
...
def multiscale_value(xmin, ymin, boxw, boxh, frame):
    ...
    level_mean = [level_e_mean, level_0_mean, level_1_mean, level_2_mean, level_3_mean]
...
if __name__ == "__main__":
    ...
    np.save("./Output/video_signal/BVP_grid_{}.npy".format(save_file_name), raw_signal)
